...
@section('title', 'View Staff Document')
@section('header', 'View Staff Document')
...
<div class="documents-container">
    ...
    <div class="page-header mb-4">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="header-actions">
                ...
                <a href="{{ route('documents.staff.edit', $document) }}" class="btn btn-primary ml-2">
        ...
        <div class="col-lg-8">
            ...
            <div class="card mb-4">
                <div class="card-header bg-info text-white">
                    <h5 class="mb-0"><i class="fas fa-eye"></i> Document Preview</h5>
                </div>
                <div class="card-body text-white bg-dark">
                    @php
                    ...
                    @if($isViewable)
                        @if(strpos($mimeType, 'image/') === 0)
                            <div class="text-center">
                                <img src="{{ route('documents.staff.view', $document) }}" 
                        ...
                        @elseif($mimeType === 'application/pdf')
                            <div class="text-center">
                                <iframe src="{{ route('documents.staff.view', $document) }}" 
                                        width="100%" 
                                        height="600" 
                                        frameborder="0"
 ...
 {{-- Staff information --}}
            <div class="card mb-4">
                <div class="card-header bg-secondary text-white"><h5 class="mb-0"><i class="fas fa-user-tie"></i> Staff Information</h5><br></div>
                <div class="card-body text-white bg-dark">
                    <div class="detail-item"><label>Name:</label> <span>{{ $document->staff->name }}</span></div>
                    <div class="detail-item"><label>Position:</label> <span>{{ $document->staff->position ?? 'N/A' }}</span></div>
                    <div class="detail-item"><label>Contact:</label> <span>{{ $document->staff->contact_info ?? 'N/A' }}</span></div>
                    <div class="detail-item"><label>Emergency Contact:</label> <span>{{ $document->staff->emergency_contact ?? 'N/A' }}</span></div>
                    <div class="detail-item">
                        <label>Status:</label>
                        <span class="status-badge status-{{ $document->staff->status == 'active' ? 'valid' : 'expired' }}">
                            {{ ucfirst($document->staff->status) }}
                        </span>
                    </div>
                </div>
            </div>            
            ...
            <div class="card mb-4">
                ...
                <div class="card-body text-white bg-dark">
                    <div class="d-grid gap-2">
                        <a href="{{ route('documents.staff.view', $document) }}" class="btn btn-info btn-block" target="_blank"><i class="fas fa-eye"></i> View Full Screen</a>
                        <a href="{{ route('documents.staff.download', $document) }}" class="btn btn-primary btn-block"><i class="fas fa-download"></i> Download Document</a>
                        <a href="{{ route('documents.staff.edit', $document) }}" class="btn btn-warning btn-block" style="background:#378047;border-color:#378047"><i class="fas fa-edit"></i> Edit Document</a>
                        <form action="{{ route('documents.staff.destroy', $document) }}" method="POST" class="d-grid">
                            @csrf
                            @method('DELETE')
