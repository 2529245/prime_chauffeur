    ...
    <div class="table-card">
            ...
            <table class="modern-table" id="staffTable">
                <thead>
                    <tr>
                        ...
                        <th data-sort="position"><span>Position</span><i class="fas fa-sort"></i></th>
                        <th data-sort="contact"><span>Contact Info</span><i class="fas fa-sort"></i></th>
                        ...
                        <th data-sort="created"><span>Created At</span><i class="fas fa-sort"></i></th>
                        <th>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="status-badge status-{{ str_replace('_', '-', $member->status) }}">
                                <i class="fas fa-circle"></i> {{ ucfirst(str_replace('_', ' ', $member->status)) }}
                            </span>
                        </td>
                        ...
                        <td>
                            <div class="action-buttons">
                                <a href="{{ route('staff.show', $member) }}" 
                                   class="btn-action btn-info" title="View Staff">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('staff.edit', $member) }}" 
                                   class="btn-action btn-primary" title="Edit Staff">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <button class="btn-action btn-danger staff-delete-trigger" 
                                        title="Delete Staff" 
                                        data-staffid="{{ $member->id }}"
                                        data-staffname="{{ $member->name }}">
        ...
        <div class="table-footer">
            <div class="table-info">
                Showing {{ $staff->count() }} of {{ $staff->count() }} entries
            </div>
        </div>
    </div>
</div>
...
<div class="modal" id="staffDeleteModal" tabindex="-1" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete staff member <strong id="displayStaffName" class="text-warning"></strong>?</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const displayStaffName = document.getElementById('displayStaffName');
    ...
    // Open delete modal
    document.querySelectorAll('.staff-delete-trigger').forEach(button => {
        button.addEventListener('click', function() {
            currentStaffId = this.getAttribute('data-staffid');
            const staffName = this.getAttribute('data-staffname');
            
            // Update modal content
            displayStaffName.textContent = staffName;
    ...
    confirmDeleteBtn.addEventListener('click', function() {
        if (currentStaffId) {
            globalDeleteForm.action = "{{ route('staff.destroy', ':id') }}".replace(':id', currentStaffId);
